...
for line in sys.stdin:
    ...
    for adj_page in adj_pages:
        # emit -> adj_page | node_name | auth
        # output.write(f'{adj_page}\t{page}\t{round(float(page_auth_score) / len(adj_pages), 2)}\n')
        print(f'{adj_page}\t{page}\t{round(float(page_auth_score) / len(adj_pages), 2)}')
